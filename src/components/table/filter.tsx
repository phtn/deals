...
const FilterComponent = <T,>({
  ...
  const activeFiltersData = useMemo(() => {
    ...
    return activeFilterColumns.map((column) => {
      ...
      const controlledFilter = columnFilters.find(
        (filter) => filter.id === column.id,
      )
      const selectedValues = (controlledFilter?.value ?? []) as (
        | string
        | number
        | boolean
      )[]
      ...
      <Popover.Trigger
        ...
        render={
          <BaseButton
            ...
            <span className='hidden font-ios text-sm capitalize opacity-90 md:flex'>
              Filter
            </span>
          </BaseButton>
        }
