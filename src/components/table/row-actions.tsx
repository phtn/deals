...
export const RowActions = <T,>({row, actionConfig}: Props<T>) => {
  ...
  const runAction = useCallback(
    async (action: ResolvedAction<T>) => {
      if (pendingActionId || resolveRowCondition(action.disabled, rowData)) {
        return
      }
  ...
  const defaultTrigger = (
    <Button
      ...
      <Icon
        name={
          pendingActionId ? 'spinner-ring' : (triggerConfig?.icon ?? 'document')
        }
              ...
              {group.items.map((action) => {
                ...
                return (
                  <DropdownMenuItem
                    ...
                    className={cn('h-10 rounded-xl', action.className)}>
  ...
  return actionConfig?.mode === 'buttons' ? defaultButtons : defaultDropdown
}
