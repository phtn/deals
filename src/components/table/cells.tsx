...
import {appendInternalPathSegment, toSafeInternalHref} from './safe-navigation'

interface CellOptions<T> {
  className?: ClassName
  formatter?: (value: unknown, context: CellContext<T, unknown>) => ReactNode
  fallback?: ReactNode
}
...
export function superCell<T>(prop: keyof T, options: CellOptions<T> = {}) {
  const CellComponent = (context: CellContext<T, unknown>) => {
    ...
    return (
      <div className={cn('font-brk text-sm', options.className)}>{value}</div>
    )
  }
...
export const firstLCell = <T, K extends keyof T>(
  ...
  const FirstLastCell = (context: CellContext<T, unknown>) => {
    ...
    return <span className={cn('leading-none', className)}>{displayValue}</span>
  }
...
export const linkText = <T, K extends keyof T>(
  prop: K,
  href: string | ((context: CellContext<T, unknown>) => string),
  formatter?: (value: string) => string,
  className?: ClassName,
  fallback?: ReactNode,
) => {
  const LinkTextComponent = (context: CellContext<T, unknown>) => {
    const rawValue = context.row.getValue(String(prop))
    if (rawValue === null || rawValue === undefined) {
    ...
    const baseHref = typeof href === 'function' ? href(context) : href
...
export function moneyCell<T>(prop: keyof T, options: CellOptions<T> = {}) {
  const MoneyCellComponent = (context: CellContext<T, unknown>) => {
    const rawValue = context.row.getValue(String(prop))
    if (rawValue === null || rawValue === undefined) {
      return fallbackContent(options.fallback, options.className)
    }
...
export function numberCell<T>(prop: keyof T, options: CellOptions<T> = {}) {
  const NumberCellComponent = (context: CellContext<T, unknown>) => {
    const rawValue = context.row.getValue(String(prop))
    if (rawValue === null || rawValue === undefined) {
      return fallbackContent(options.fallback, options.className)
    }
...
export const dateCell = <T,>(
  ...
  const cell = superCell<T>(prop, {
    ...
    formatter: (value) => {
      ...
      return Number.isFinite(timestamp) ? formatEventDate(timestamp, '') : '—'
    },
  })
  ...
  const cell = superCell<T>(prop, {
    ...
    formatter: (value) => {
      ...
      const iconName = enabled
        ...
        : (icons.falseIcon ?? icons.falseLabel ?? 'close')

      return (
        <div className='flex h-6 items-center'>
          <Icon
            name={iconName}
            className={cn('size-6', enabled && 'text-emerald-500')}
          />
        </div>
      )
    },
...
const StatusBadge = ({status}: {status: string}) => (
  <span
    ...
    <span className='size-2 rounded-full bg-amber-200' aria-hidden='true' />
...
export const editableStatusCell = <T,>(
  ...
  const EditableStatusComponent = (context: CellContext<T, unknown>) => {
    ...
    return (
      <button
        ...
        aria-pressed={value}
        aria-label={
          failed ? 'Status update failed. Try toggling again.' : undefined
        }
    ...
    return (
      <div className='flex justify-center px-0.5'>
        <Button
          ...
          data-error={failed || undefined}
          id={failed ? 'Update failed. Try again.' : `Current: ${currentLabel}`}
          className={cn(
...
export const editableCell = <T,>(
  ...
  const EditableCellComponent = (context: CellContext<T, unknown>) => {
    const originalValue = String(context.row.getValue(String(prop)) ?? '')
    const row = context.row.original
    ...
    return (
      <button
        ...
        aria-label={
          failed ? 'Edit value. The previous update failed.' : undefined
        }
        data-error={failed || undefined}
        className={cn(
  ...
  EditableCellComponent.displayName = `EditableCell(${String(prop)})`
...
export const createUserCell = <T,>(options: UserCellOptions<T>) => {
  const UserCellComponent = (context: CellContext<T, unknown>) => {
    const row = context.row.original
