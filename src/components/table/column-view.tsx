...
const ColumnViewComponent = <T,>({
  cols,
  columnVisibility,
  onColumnVisibilityChange,
}: Props<T>) => {
  const hideableColumns = useMemo(
    () => cols.filter((column) => column.getCanHide()),
    [cols],
  )
  ...
  const invisibleColumns = hideableColumns.filter(
    (column) => !getIsVisible(column),
  )
      ...
      <Menu.Trigger
        render={
          <Button
            ...
            <span className='hidden font-ios text-sm opacity-90 md:flex'>
              Columns
            </span>
          </Button>
        }>
                ...
                return (
                  <Menu.CheckboxItem
                    ...
                    className={cn(
                      ...
                      isVisible &&
                        'bg-transparent not-italic opacity-100 dark:bg-transparent dark:text-white',
                    )}
                    checked={isVisible}
                    onCheckedChange={(value) => handleToggle(column.id, value)}>
                    <span>{getColumnHeaderText(column)}</span>
                    <Icon
                      name={isVisible ? 'check' : 'eye-close'}
                      className='size-4'
                    />
                  </Menu.CheckboxItem>
                )
